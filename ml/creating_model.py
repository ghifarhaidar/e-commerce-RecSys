...
class SeqModelWrapper(mlflow.pyfunc.PythonModel):
    ...
    def load_context(self, context):
        ...
        self.hparams = prepare_hparams(context.artifacts["yaml_file"])
        self.input_creator = SequentialIterator
        
        self.item_vocab_file = context.artifacts["item_vocab_dir"]
        self.cate_vocab_file = context.artifacts["cate_vocab_dir"]

        # Load the model from saved artifacts
        self.model = SeqModel(self.hparams, self.input_creator, seed=RANDOM_SEED)
    ...
    def process_and_write_to_file(self, model_input, filename, item_vocab_file, cate_vocab_file):
        ...
        with open(self.item_vocab_file, 'rb') as f:
            self.item_voc = pickle.load(f)

        with open(self.cate_vocab_file, 'rb') as f:
            self.cate_voc = pickle.load(f)

        with open(filename, 'w') as f:
       
            label = 0 # Prepare the label value
            # Extracting values 
            for _, row in model_input.iterrows():
                ...
                timestamp_unix = self.convert_to_unix(timestamp) # Convert timestamp to Unix time
                ...
                history_item_strs = [
                    "Item" + str(item) if "Item" + str(item) in self.item_voc else "default_pid"
                    for item in history_item_ids
                ]
                history_category_strs = [
                    "Cate" + str(cat) if "Cate" + str(cat) in self.cate_voc else "default_cat"
                ...
                for item_id, category_id in zip(all_product_ids, all_category_ids):
                    ...
                    item_str = "Item" + str(item_id) if "Item" + str(item_id) in self.item_voc else "default_pid"
                    category_str = "Cate" + str(category_id) if "Cate" + str(category_id) in self.cate_voc else "default_cat"
    ...
    def predict(self, context, model_input):
        ...
        model_input = self.process_and_write_to_file(model_input , "input.txt", self.item_vocab_file, self.cate_vocab_file)
        


        self.model = self.model.predict("input.txt" , "output.txt")
